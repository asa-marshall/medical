...
if(isset($_POST['selectedIDs'])) {
    //selectedIDs is an array of string ["23","24"]
    $selectedIDs= json_decode($_POST['selectedIDs']);
    $str =implode("','",$selectedIDs);
    $sql = "$sql AND ApplicantID in ('$str')";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($result3->num_rows > 0) {
            while ($row3 = $result3->fetch_assoc()) {
                ...
                if(count($guardianEmails) > 0)
                {
                    $temp['GuardianName']="$GuardianSalutaion $GuardianFN $GuardianLN";
                    $temp['emails'] = $guardianEmails;
